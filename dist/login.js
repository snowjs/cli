'use strict';
var __awaiter =
  (this && this.__awaiter) ||
  function(thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function(resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator['throw'](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : new P(function(resolve) {
              resolve(result.value);
            }).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function(thisArg, body) {
    var _ = {
        label: 0,
        sent: function() {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: []
      },
      f,
      y,
      t,
      g;
    return (
      (g = { next: verb(0), throw: verb(1), return: verb(2) }),
      typeof Symbol === 'function' &&
        (g[Symbol.iterator] = function() {
          return this;
        }),
      g
    );
    function verb(n) {
      return function(v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError('Generator is already executing.');
      while (_)
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y['return']
                  : op[0]
                  ? y['throw'] || ((t = y['return']) && t.call(y), 0)
                  : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
var _this = this;
exports.__esModule = true;
var providers_1 = require('./providers');
var utils_1 = require('./utils');
exports['default'] = function() {
  return __awaiter(_this, void 0, void 0, function() {
    var question,
      provider,
      _a,
      projectsStr,
      projects,
      projectId,
      clusterData,
      _b,
      location,
      name,
      token;
    return __generator(this, function(_c) {
      switch (_c.label) {
        case 0:
          question = 'Which cloud provider do you want to login to';
          return [
            4 /*yield*/,
            utils_1.pickOne(question, providers_1['default'])
          ];
        case 1:
          provider = _c.sent();
          _a = provider;
          switch (_a) {
            case 'minikube':
              return [3 /*break*/, 2];
            case 'gcp':
              return [3 /*break*/, 4];
            case 'digitalocean':
              return [3 /*break*/, 10];
          }
          return [3 /*break*/, 14];
        case 2:
          return [
            4 /*yield*/,
            utils_1.run('kubectl config use-context minikube')
          ];
        case 3:
          _c.sent();
          return [3 /*break*/, 15];
        case 4:
          return [4 /*yield*/, utils_1.run('gcloud auth login')];
        case 5:
          _c.sent();
          return [
            4 /*yield*/,
            utils_1.run('gcloud projects list --format json')
          ];
        case 6:
          projectsStr = _c.sent().stdout;
          projects = JSON.parse(projectsStr);
          if (projects.length === 0) {
            utils_1.logError('No projects exist. Create a project.');
            return [2 /*return*/];
          }
          projectId = projects[0].projectId;
          return [
            4 /*yield*/,
            utils_1.run('gcloud config set project ' + projectId)
          ];
        case 7:
          _c.sent();
          return [
            4 /*yield*/,
            utils_1.run('gcloud container clusters list --format="json"')
          ];
        case 8:
          clusterData = _c.sent().stdout;
          (_b = JSON.parse(clusterData)[0]),
            (location = _b.location),
            (name = _b.name);
          return [
            4 /*yield*/,
            utils_1.run(
              'gcloud container clusters get-credentials ' +
                name +
                ' --zone "' +
                location +
                '"'
            )
          ];
        case 9:
          _c.sent();
          return [3 /*break*/, 15];
        case 10:
          utils_1.logInfo(
            'Obtain a Digital Ocean access token here: https://cloud.digitalocean.com/account/api/tokens'
          );
          return [
            4 /*yield*/,
            utils_1.askForInput('Enter your digital ocean access token')
          ];
        case 11:
          token = _c.sent();
          return [4 /*yield*/, utils_1.run('doctl auth init -t ' + token)];
        case 12:
          _c.sent();
          return [
            4 /*yield*/,
            utils_1.run('doctl k8s cluster kubeconfig save snow-cluster')
          ];
        case 13:
          _c.sent();
          return [3 /*break*/, 15];
        case 14:
          {
            utils_1.logError('No valid cloud provider selected.');
          }
          _c.label = 15;
        case 15:
          return [2 /*return*/];
      }
    });
  });
};
//# sourceMappingURL=login.js.map
